...
int main(void) {
    ...
    // cudaMallocManaged((void **)&device_array_a, num_bytes);
    // cudaMallocManaged((void **)&device_array_b, num_bytes);
    // cudaMallocManaged((void **)&device_array_c, num_bytes);
    ...
    if (array_a == 0 || array_b == 0 || array_c == 0){
        printf("culdn't allocate memory\n");
        return 1;
    }
    ...
    //! WE NEED THIS BECAUSE in the original version kernel and memcpy are in a stream (which is a queue)
