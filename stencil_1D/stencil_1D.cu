...
__global__ void stencil_1d_improved(int *in, int *out) {
    __shared__ int temp[BLOCK_SIZE + RADIUS * 2]; /* FIXME #2*/

    int gindex = threadIdx.x + (blockIdx.x * blockDim.x) + RADIUS; /* FIXME #3*/
    int lindex = threadIdx.x + RADIUS;                             /* FIXME #4 */

    // Read input elements into shared memory
    // Each thread reads one element from global to shared memory
    temp[lindex] = in[gindex];

    // Load ghost cells (halos)
    if (threadIdx.x < RADIUS) {
        temp[lindex - RADIUS] = in[gindex - RADIUS];
        temp[lindex + BLOCK_SIZE] = in[gindex + BLOCK_SIZE]; // Copy halos at the end of the local scope
    }
